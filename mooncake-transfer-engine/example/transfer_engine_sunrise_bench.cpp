...
#if defined(USE_CUDA) || defined(USE_MUSA) || defined(USE_HIP) || defined(USE_SUNRISE)
#include <cassert>
...
DEFINE_uint64(buffer_size, 1ull << 20, "total size of data buffer"); // 4GB for 4MB block_size test
...
DEFINE_bool(copy_dram, false, "if use transfer path: vram1->dram1->rdma1->rdma2->dram2->vram2");
#ifdef USE_SUNRISE
DEFINE_bool(sunrise_use_mapped_host_vram, true,
...
static void *allocateMemoryPool(size_t size, int buffer_id,
    ...
    if (from_vram) {
        ...
        if (FLAGS_sunrise_use_mapped_host_vram) {
            checkTangError(tangHostAlloc(&d_buf, size, tangHostAllocMapDeviceMemory),
                           "Failed to allocate mapped host memory");
        } else {
            checkTangError(tangSetDevice(gpu_id), "Failed to set device");
            checkTangError(tangMalloc(&d_buf, size), "Failed to allocate device VRAM");
        }
#endif
        LOG(INFO) << "Allocated memory on GPU " << gpu_id << ", addr=" << d_buf << ", size=" << (size>>20) << "MB";
        if (FLAGS_init_mem) {
            // memset(d_buf, 0, size);
...
Status initiatorWorker(TransferEngine *engine, SegmentID segment_id,
    ...
    while (running.load(std::memory_order_acquire)) {
        ...
        for (int i = 0; i < FLAGS_batch_size; ++i) {
            ...
            // batch into one memcpy (interleaved i*threads+tid prevents merging).
            entry.source = (uint8_t *)(addr) +
                           FLAGS_block_size *
                               (thread_id * FLAGS_batch_size + i);
...
static void registerBuffers(mooncake::tent::TransferEngine *engine,
    ...
    for (int i = 0; i < buffer_num; ++i) {
        ...
        auto status = engine->registerLocalMemory(addr[i], FLAGS_buffer_size, options);
...
std::shared_ptr<mooncake::tent::Config> createTentConfig() {
    ...
    // P2P mode builds segment name from rpc bind address, not from this field alone.
    // Without rpc_server_hostname/port, TE discovers another NIC (e.g. 192.168.*)
    // and the peer cannot connect using -segment_id=10.21.60.35:port.
    {
    ...
    // Minimal diagnostics during initiator/target perf runs (not "verbose" TE logs).
    ...
    bool enable_sunrise_link = !(FLAGS_use_vram && FLAGS_sunrise_use_mapped_host_vram);
    config->set("transports/sunrise_link/enable", enable_sunrise_link);

    LOG(INFO) << "TENT config: RDMA "
              << (enable_rdma ? "enabled" : "disabled")
              << ", SUNRISE_LINK "
...
void initiatorWorker(mooncake::tent::TransferEngine *engine,
    ...
    while (running.load(std::memory_order_acquire)) {
        ...
        const auto batch_wait_start = std::chrono::steady_clock::now();
        // Poll overall batch status (same aggregation as waitTransferCompletion).
        while (true) {
            if (!running.load(std::memory_order_acquire)) {
                LOG(ERROR)
                    << "Worker " << thread_id
                ...
                if (!fr.ok()) {
                    LOG(WARNING) << "freeBatch after shutdown: " << fr.ToString();
                }
                total_batch_count.fetch_add(batch_count);
                return;
            }
            ...
            // `overall.s == TransferStatusEnum::COMPLETED` always false even when
            // the underlying value is COMPLETED (see tent/common/types.h).
            const int ost = static_cast<int>(overall.s);
            const int kComp = static_cast<int>(
                mooncake::tent::TransferStatusEnum::COMPLETED);
            const int kFail = static_cast<int>(
...
int initiator() {
    ...
    for (int attempt = 0; attempt < 6; ++attempt) {
        ...
        if (attempt > 0) {
            ...
            if (!reopen.ok()) {
                LOG(WARNING) << "Re-open remote segment failed: "
                             << reopen.ToString();
            }
        }
