...
#ifdef USE_SUNRISE
#include "/data/liujialai/codes/practice/pccl_connection/pccl_connection.h"
#endif
...
static void *allocateMemoryPool(size_t size, int socket_id,
    ...
    void* ptr = pccl_connection().allocGpuMemory(size);
    printf("【sunrise】allocate memory from pccl_connection lib, ptr=%p \n", ptr);
    if (ptr == nullptr) {
        ptr = numa_alloc_onnode(size, socket_id);
        printf("【sunrise】allocate gpu memory fail, ptr=%p \n", ptr);
        return ptr;
    }
    return ptr;
#endif
...
int initiatorWorker(TransferEngine *engine, SegmentID segment_id, int thread_id,
    ...
    {
        LOG(INFO) << "Stage 1: Write Data" << " into addr:" << addr;
    ...
    {
        LOG(INFO) << "Stage 2: Read Data" << " on addr:" << (void*)((uint8_t *)(addr) + kDataLength);
        auto batch_id = engine->allocateBatchID(1);
        Status s;
...
int initiator() {
    ...
    std::string name_prefix = FLAGS_use_vram ? "ptpu:" : "cpu:";
