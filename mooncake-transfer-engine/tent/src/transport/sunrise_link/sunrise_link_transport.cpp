...
// Include SunriseLink runtime API (PTML and Tang)
#ifdef USE_SUNRISE
...
static std::string TangRtSharedObjectPath(const char* soname) {
    ...
    std::string r =
        (root && root[0]) ? std::string(root) : std::string("/usr/local/tangrt");
...
static tangMemcpyKind MemcpyKindForPointers(tangMemoryType st, tangMemoryType dt) {
...
static void NormalizeMappedHostPointer(void** ptr, tangPointerAttributes* attr) {
...
static tangError_t C2cMemcpyWithPeerMap(int src_dev, int dst_dev, const void* src,
                                        void* dst, size_t len,
                                        int port_src_to_dst, int port_dst_to_src) {
...
static ptmlReturn_t (*ptmlPtlinkPhytopoDetect)(ptmlDevice_t dev, int len, void* data) =
    nullptr;
...
Status SunriseLinkTransport::install(std::string& local_segment_name,
    ...
    async_memcpy_threshold_ =
        conf_ ? conf_->get("transports/sunrise_link/async_memcpy_threshold", 0) * 1024
              : 0;
    host_register_ =
        conf_ ? conf_->get("transports/sunrise_link/host_register", false) : false;
...
#ifdef USE_SUNRISE
    ptml_handle_ = dlopen(TangRtSharedObjectPath("libptml_shared.so").c_str(),
                          RTLD_NOW);
    if (!ptml_handle_) {
        char* error = dlerror();
...
#define LOAD_PTML_SYM(handle, symbol, funcptr) \
    ...
    LOAD_PTML_SYM(ptml_handle_, "ptmlPtlinkPhytopoDetect", ptmlPtlinkPhytopoDetect);
    ...
    return Status::OK();
#else
    LOG(INFO) << "SunriseLink transport stub mode (USE_SUNRISE not defined)";
    return Status::OK();
#endif
}
...
Status SunriseLinkTransport::detectTopology() {
    ...
    if (pret != PTML_SUCCESS || device_count <= 0) {
        LOG(ERROR) << "ptmlDeviceGetCount failed, ret=" << pret << " count=" << device_count;
        return Status::InternalError("ptmlDeviceGetCount failed");
    }

    std::vector<ptPhyTopo_t> phytopo(
        static_cast<size_t>(device_count) * kMaxPhytopoPorts);

    for (int dev = 0; dev < device_count; ++dev) {
        memset(phytopo.data() + static_cast<size_t>(dev) * kMaxPhytopoPorts, 0,
               kMaxPhytopoPorts * sizeof(ptPhyTopo_t));

        pret = ptmlPtlinkPhytopoDetect(dev, kMaxPhytopoPorts * sizeof(ptPhyTopo_t),
                                       phytopo.data() +
                                           static_cast<size_t>(dev) * kMaxPhytopoPorts);
        ...
        for (int p = 0; p < kMaxPhytopoPorts; ++p) {
            ...
            LOG(INFO) << "PTLink topo dev=" << dev << " port=" << lp << " link " << lc
                      << " -> " << rc << " (remote_port=" << static_cast<int>(row[p].remote_port)
                      << ")";
        }
    ...
    return Status::OK();
#else
    LOG(INFO) << "SunriseLink topology detection stub mode";
    return Status::OK();
#endif
}

Status SunriseLinkTransport::allocateSubBatch(SubBatchRef& batch, size_t max_size) {
    auto shm_batch = Slab<SunriseLinkSubBatch>::Get().allocate();
    if (!shm_batch) {
        return Status::InternalError("Unable to allocate SunriseLink sub-batch");
    }
    batch = shm_batch;
...
Status SunriseLinkTransport::freeSubBatch(SubBatchRef& batch) {
    ...
    if (!shm_batch) {
        return Status::InvalidArgument("Invalid SunriseLink sub-batch" LOC_MARK);
    }
    shm_batch->task_list.clear();
...
Status SunriseLinkTransport::submitTransferTasks(SubBatchRef batch,
    ...
    for (auto& request : request_list) {
        ...
        if (request.target_id != LOCAL_SEGMENT_ID) {
            auto status =
                relocateRemoteAddress(target_addr, request.length, request.target_id,
                                      &target_gpu_id);
        ...
        if (task.tang_gpu_id < 0 && request.remote_gpu_id >= 0) {
            task.tang_gpu_id = request.remote_gpu_id;
        }
...
Status SunriseLinkTransport::startTransfer(SunriseLinkTask* task,
    ...
    (void)stream;
    // Disable async memcpy for completion bookkeeping: async success returned OK
    // with status_word PENDING and relied on tangStreamQuery(stream), which often
    // never reached idle when stream/device context disagreed (multi-GPU benches).
    ...
    if (task->is_tang_ipc && task->tang_gpu_id >= 0) {
        ...
        if (task->request.opcode == Request::WRITE) {
            local_dev = infer_local_dev(src);
        } else {
            local_dev = infer_local_dev(dst);
        }
        if (local_dev < 0) local_dev = current_dev;

        int src_dev = (task->request.opcode == Request::WRITE) ? local_dev : remote_dev;
        int dst_dev = (task->request.opcode == Request::WRITE) ? remote_dev : local_dev;
        ...
        if (src_dev == dst_dev) {
            ...
            if (ret != tangSuccess) {
                LOG(ERROR) << "SunriseLink IPC same-device memcpy failed, tang ret="
        ...
        if (ret != tangSuccess) {
            int p_sd = -1;
            int p_ds = -1;
            auto it_sd = c2c_port_by_pair_.find(std::make_pair(src_dev, dst_dev));
            if (it_sd != c2c_port_by_pair_.end()) p_sd = it_sd->second;
            auto it_ds = c2c_port_by_pair_.find(std::make_pair(dst_dev, src_dev));
            if (it_ds != c2c_port_by_pair_.end()) p_ds = it_ds->second;
            ret = C2cMemcpyWithPeerMap(src_dev, dst_dev, src, dst, len, p_sd, p_ds);
        }
        if (ret != tangSuccess) {
            LOG(ERROR) << "SunriseLink IPC peer copy failed, tang ret=" << ret << " "
                       << tangGetErrorString(ret) << " src_dev=" << src_dev
                       << " dst_dev=" << dst_dev << " remote_dev=" << remote_dev
                       << " local_dev=" << local_dev;
            task->status_word = TransferStatusEnum::FAILED;
            return Status::InternalError("SunriseLink transfer failed");
        }
    ...
    int remote_dev_hint = task->tang_gpu_id >= 0 ? task->tang_gpu_id
                                                 : task->request.remote_gpu_id;
    if (task->request.opcode == Request::WRITE) {
    ...
    if (task->request.remote_gpu_id >= 0 && task->request.remote_port >= 0) {
        (void)task->request.remote_gpu_id;
    }

    tangError_t ret = tangSuccess;

    if (task->is_tang_ipc && src_dev >= 0 && dst_dev >= 0 && src_dev == dst_dev) {
        ScopedTangDeviceOps ctx_lock(src_dev, dst_dev);
        tangSetDevice(src_dev);
        if (can_async) {
            ret = tangMemcpyAsync(dst, src, len, tangMemcpyDeviceToDevice, stream);
            if (ret == tangSuccess) {
                return Status::OK();
            }
        }
        ...
        if (ret != tangSuccess) {
            LOG(ERROR) << "SunriseLink IPC local-device memcpy failed, tang ret=" << ret
                       << " " << tangGetErrorString(ret) << " dev=" << src_dev;
            task->status_word = TransferStatusEnum::FAILED;
            return Status::InternalError("SunriseLink transfer failed");
        }
        task->transferred_bytes = len;
    ...
    if (src_dev >= 0 && dst_dev >= 0 &&
        ...
        if (task->request.remote_port >= 0) {
            ports_sd = {task->request.remote_port};
            ports_ds = {task->request.remote_port};
        }

        constexpr size_t kMultiPortThreshold = 4ull << 20;  // 4 MiB
        const bool enable_port_striping =
            conf_ ? conf_->get("transports/sunrise_link/enable_port_striping", false)
                  : false;
        if (enable_port_striping && !ports_sd.empty() && ports_sd.size() > 1 &&
            ...
            for (size_t lane = 0; lane < lanes; ++lane) {
                ...
                ret = C2cMemcpyWithPeerMap(
                    src_dev, dst_dev,
                    static_cast<char*>(src) + lane_offset,
        ...
        auto peer_sync = [&]() -> tangError_t {
            ...
            if (r != tangSuccess) {
                ...
                r = C2cMemcpyWithPeerMap(src_dev, dst_dev, src, dst, len, p_sd, p_ds);
            }
            return r;
        };
...
Status SunriseLinkTransport::getTransferStatus(SubBatchRef batch, int task_id,
    ...
    if (task.status_word == TransferStatusEnum::PENDING) {
        ...
        if (stream) {
            ...
            LOG(ERROR) << "SunriseLink getTransferStatus: PENDING with null stream; "
...
Status SunriseLinkTransport::addMemoryBuffer(BufferDesc& desc,
    ...
    auto status = registerMemory(reinterpret_cast<void*>(desc.addr), desc.length);
    if (!status.ok()) return status;
    if (location.type() == "cuda" || location.type() == "npu") {
        ...
        auto ret = tangIpcGetMemHandle(&handle, reinterpret_cast<void*>(desc.addr));
        if (ret != tangSuccess) {
            LOG(ERROR) << "tangIpcGetMemHandle failed: " << ret << " "
                       << tangGetErrorString(ret);
            return Status::InternalError("tangIpcGetMemHandle failed");
        }
        desc.shm_path = serializeBinaryData(&handle, sizeof(tangIpcMemHandle_t));
    } else if (location.type() == "cpu" || location.type() == kWildcardLocation) {
        registered_memory_gpu_id_[reinterpret_cast<void*>(desc.addr)] = -1;
        if (host_register_) {
            tangError_t hr = tangHostRegister(reinterpret_cast<void*>(desc.addr),
                                              desc.length, tangHostRegisterDefault);
            if (hr != tangSuccess) {
                LOG(ERROR) << "tangHostRegister failed: " << hr << " "
                           << tangGetErrorString(hr);
                return Status::InternalError("tangHostRegister failed");
            }
        }
    }
...
Status SunriseLinkTransport::registerMemory(void* addr, size_t size) {
    ...
    LOG(INFO) << "Registered memory: addr=" << addr << " size=" << size;
    return Status::OK();
#else
    (void)addr;
    (void)size;
...
Status SunriseLinkTransport::deregisterMemory(void* addr) {
    ...
    if (it != registered_memory_.end()) {
        ...
        LOG(INFO) << "Deregistered memory: addr=" << addr;
    }
    return Status::OK();
#else
    (void)addr;
...
Status SunriseLinkTransport::removeMemoryBuffer(BufferDesc& desc) {
    ...
    if ((location.type() == "cpu" || location.type() == kWildcardLocation) &&
        host_register_) {
        tangHostUnregister(reinterpret_cast<void*>(desc.addr));
    }
...
Status SunriseLinkTransport::relocateRemoteAddress(uint64_t& dest_addr,
    ...
    for (auto& entry : tl_relocate_map[target_id]) {
        if (entry.first <= dest_addr &&
            ...
            dest_addr = dest_addr - entry.first + reinterpret_cast<uint64_t>(entry.second.dev_ptr);
    ...
    if (!buffer || buffer->shm_path.empty()) {
        return Status::InvalidArgument("Requested address is not in registered Sunrise buffer" LOC_MARK);
    }
    ...
    if (!relocate_map_[target_id].count(buffer->addr)) {
        ...
        if (location.type() != "cuda" && location.type() != "npu") {
            return Status::InvalidArgument(
                "Requested address is not in registered CUDA/NPU buffer" LOC_MARK);
        }
        std::vector<unsigned char> decoded;
        deserializeBinaryData(buffer->shm_path, decoded);
        if (decoded.size() != sizeof(tangIpcMemHandle_t)) {
            return Status::InvalidArgument("Invalid Sunrise IPC handle size" LOC_MARK);
        }
        tangIpcMemHandle_t handle;
        ...
        if (sd != tangSuccess) {
            LOG(ERROR) << "tangSetDevice before tangIpcOpenMemHandle failed: " << sd
                       << " " << tangGetErrorString(sd);
        }
        tangError_t ret =
            tangIpcOpenMemHandle(&opened_ptr, handle, tangIpcMemLazyEnablePeerAccess);
        if (saved_dev >= 0) {
    ...
    dest_addr = dest_addr - buffer->addr + reinterpret_cast<uint64_t>(opened_ptr);
    return Status::OK();
#else
    (void)dest_addr;
    (void)length;
