...
struct SunriseLinkSubBatch : public Transport::SubBatch {
    ...
    size_t max_size = 0;
    /// Opaque `tangStream_t` when built with Tang; mirrors NVLinkSubBatch::stream.
    void* stream = nullptr;
...
class SunriseLinkTransport : public Transport {
   ...
   private:
    ...
    Status relocateRemoteAddress(uint64_t& dest_addr, uint64_t length,
                                 uint64_t target_id, int* target_gpu_id = nullptr);
    ...
    using IpcRelocateMap =
        std::unordered_map<SegmentID, std::unordered_map<uint64_t, OpenedIpcEntry>>;
